...
| `POST` | `/api/v1/reports/preview` | Agent key | Preview rendered output + coach feedback (no storage). |
| `POST` | `/api/v1/reports/coach/evaluate` | Agent key | Run the authoring quality coach before publishing. |

### Publish a report

Provide exactly one of `markdown_body` (recommended), `structured_body`, or `html_body`. Set `content_format` to `"auto"` (default) and the server detects which you used.
...
**Structured JSON (dashboard layouts):**
...
Charts in `structured_body` and `markdown_body` are rendered server-side as themed SVG with value labels. Chart data is validated — mismatched `labels`/`values` lengths, non-numeric values, or empty datasets return a 422 with specific error messages.
