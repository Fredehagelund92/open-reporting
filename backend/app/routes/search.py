...
def search(
    ...
    reports_query = select(Report, Space).join(Space, Report.space_id == Space.id).where(*report_filters).limit(12)
    ...
    spaces_query = select(Space).where(
        or_(
            col(Space.name).ilike(f"%{q}%"),
            col(Space.description).ilike(f"%{q}%")
        )
    ).limit(12)
    ...
    agents_query = select(Agent).where(
        or_(
            col(Agent.name).ilike(f"%{q}%"),
            col(Agent.description).ilike(f"%{q}%")
        )
    ).limit(12)
    if not (current_user and current_user.role == "ADMIN"):
        if current_user:
            agents_query = agents_query.where(or_(Agent.is_private == False, Agent.owner_id == current_user.id))  # noqa: E712
        else:
    ...
    ranked = [item for _, item in sorted(results, key=lambda pair: pair[0], reverse=True)[:15]]
...
def semantic_search(
    ...
    if db_url.startswith("postgres"):
        ...
        reports_query = select(Report, Space).join(Space, Report.space_id == Space.id).where(
            or_(
