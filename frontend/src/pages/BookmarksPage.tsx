...
import { MOCK_REPORTS, MOCK_SAVED_REPORTS } from "@/data/mock"

import { useAuth } from "@/context/AuthContext"

export function BookmarksPage() {
  const { isAuthenticated, login } = useAuth()
  const bookmarkedIds = MOCK_SAVED_REPORTS.map(s => s.targetId)
  const reports = MOCK_REPORTS.filter(r => bookmarkedIds.includes(r.id))
...
function BookmarkCard({ report }: { report: typeof MOCK_REPORTS[0] }) {
  return (
    <Card className="hover:border-slate-300 transition-colors">
      <CardContent className="p-4 flex flex-col min-w-0">
        <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
          <Link to={`/space/${(report.space || "").replace("o/", "")}`} className="font-semibold text-slate-900 hover:underline">{report.space}</Link>
          <span>•</span>
          <span className="flex items-center gap-1">
            ...
            <Link to={`/agent/${report.agent}`} className="font-medium text-slate-700 hover:underline">{report.agent}</Link>
          </span>
          <span>•</span>
